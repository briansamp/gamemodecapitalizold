...
CreateDepositorJobPoint()
{
	...
	format(strings, sizeof(strings), "Depositor Jobs\ngunakan commando '/getjob' untuk menjadi pegawa depositor");
...
CMD:restockmoneyatm(playerid, params[])
{
	...
	if(pData[playerid][pJob] == 11 || pData[playerid][pJob2] == 11)
	{
		if(id > -1)
		{
			...
			pData[playerid][pJobTime] = 2800;
		    new rand = Random(1984589, 2239315);
			Server_AddMoney(rand);
			pData[playerid][pJobTime] = 2800;
			AddPlayerSalary(playerid, "Depositor(Restock)", 4500);
			SCM(playerid, COLOR_JOB, "JOB: "WHITE_E"You have "GREEN_E"succesfully "WHITE_E"restock ATM money, in your payment in '"YELLOW_E"/mysalary"WHITE_E"'");
    ...
    if(!IsValidTimer(pData[playerid][pActivity])) return 0;
	{
	    ...
	    if(pData[playerid][pActivityTime] >= 100)
	    {
		    if(rand == 7)
		    {
		        ...
		        SCM(playerid, COLOR_JOB, "JOB: "WHITE_E"ATM Money Stock: "GREEN_E"$00,00");
		        TogglePlayerControllable(playerid, 1);
				return 1;
			}
		}
	    ...
	    if(pData[playerid][pActivityTime] >= 100)
	    {
		    if(rand == 9)
		    {
		        ...
		        SCM(playerid, COLOR_JOB, "JOB: "WHITE_E"This ATM "RED_E"Down "WHITE_E"please repair");
		        TogglePlayerControllable(playerid, 1);
				return 1;
			}
		}
