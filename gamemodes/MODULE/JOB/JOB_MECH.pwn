...
CMD:upv(playerid)
{
	new id = GetNearestWorkshop(playerid, 8);
	if(pData[playerid][pJob] == 2 || pData[playerid][pJob2] == 2)
	{
		if(IsPlayerInRangeOfPoint(playerid, 50.0, wsData[id][WsRepX], wsData[id][WsRepY], wsData[id][WsRepZ]))
		{
			...
			{
				new Dstring[800];
				if(pData[playerid][pMechSkill] >= 1)
				{
					format(Dstring, sizeof(Dstring), "Upgrade Name\tComponent\n\
					Upgrade Enggine\t500\n");
					format(Dstring, sizeof(Dstring), "%sUpgrade Body\t500\n", Dstring);
					ShowPlayerDialog(playerid, DIALOG_UPGRADE, DIALOG_STYLE_TABLIST_HEADERS, "Mech Upgrade", Dstring, "Service", "Cancel");
				}
				else
				{
					Error(playerid, "Kamu belum Mechanic Lvl 5.");
				}
			}
			else
			{
...
function EngineFix(playerid, vehicleid)
{
	...
	if(pData[playerid][pJob] == 2 || pData[playerid][pJob2] == 2)
	{
		if(GetNearestVehicleToPlayer(playerid, 3.8, false) == vehicleid)
		{
			if(pData[playerid][pActivityTime] >= 100 && IsValidVehicle(vehicleid))
			{
				if(pvData[vehicleid][cMesinUpgrade] != 1)
				{
...
function UpgradeEngine(playerid, vehicleid)
{
	if(!IsPlayerConnected(playerid)) return 0;
	//if(!IsValidVehicle(vehicleid)) return 0;
	if(!IsValidTimer(pData[playerid][pMechanic])) return 0;
	if(pData[playerid][pJob] == 2 || pData[playerid][pJob2] == 2)
	{
		if(GetNearestVehicleToPlayer(playerid, 3.8, false) == vehicleid)
		{
			if(pData[playerid][pActivityTime] >= 100 && IsValidVehicle(vehicleid))
			{
				...
				InfoTD_MSG(playerid, 8000, "Upgrade engine done!");
				PlayerPlaySound(playerid,1133,0.0,0.0,0.0);
				pData[playerid][pMechVeh] = vehicleid;
				KillTimer(pData[playerid][pMechanic]);
				pData[playerid][pActivityTime] = 0;
				HidePlayerProgressBar(playerid, pData[playerid][activitybar]);
				PlayerTextDrawHide(playerid, ActiveTD[playerid]);
				pvData[vehicleid][cMesinUpgrade] = 1;
				ClearAnimations(playerid);
				pData[playerid][pMechSkill] ++;
				return 1;
			}
			...
			{
				Error(playerid, "Upgrade Engine fix fail! Anda tidak berada didekat kendaraan tersebut!");
				KillTimer(pData[playerid][pMechanic]);
				HidePlayerProgressBar(playerid, pData[playerid][activitybar]);
				PlayerTextDrawHide(playerid, ActiveTD[playerid]);
				pData[playerid][pActivityTime] = 0;
	...
	if(pData[playerid][pJob] == 2 || pData[playerid][pJob2] == 2)
	{
		if(GetNearestVehicleToPlayer(playerid, 3.8, false) == vehicleid)
		{
			if(pData[playerid][pActivityTime] >= 100 && IsValidVehicle(vehicleid))
			{
				...
				HidePlayerProgressBar(playerid, pData[playerid][activitybar]);
				PlayerTextDrawHide(playerid, ActiveTD[playerid]);
				pvData[vehicleid][cBodyUpgrade] = 1;
				ClearAnimations(playerid);
				pData[playerid][pMechSkill] ++;
				return 1;
			}
