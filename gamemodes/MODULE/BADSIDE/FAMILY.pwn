...
CMD:founinvite(playerid, params[])
{
	...
	if(pData[playerid][pFamilyRank] < 6)
		return Error(playerid, "You must family level 6!");
	
	new player[24];
    if(sscanf(params, "s[24]", player))
        return Usage(playerid, "/founinvite [PartOfName]");
    ...
    foreach(new ii : Player)
	{
		...
		{
			Error(playerid, "Player is online, you can use /funinvite on him.");
	  		return true;
	  	}
	}
	...
	mysql_tquery(g_SQL, query, "OkickMem", "is", playerid, player);
	return 1;
}
function OkickMem(playerid, NameToKick[])
{
	if(cache_num_rows() < 1)
	{
		return Error(playerid, "Account "YELLOW_E"'%s' "WHITE_E"does not exist.", NameToKick);
	}
	else
	{
		new RegID;
		...
		new query[512];
		Servers(playerid, "Anda berhasil mengeluarkan "YELLOW_E"'%s' "WHITE_E"dari family.", NameToKick);
		mysql_format(g_SQL, query, sizeof(query), "UPDATE players SET family=-1 WHERE reg_id=%d", RegID);
		mysql_tquery(g_SQL, query);
	}
	return 1;
}
...
CMD:oof(playerid, params[])
{
    ...
    if(sscanf(params,"s[128]",text))
        return Usage(playerid, "/oof(family) [ooc text]");
