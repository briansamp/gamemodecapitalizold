...
stock eventSpawn(playerid)
{
	if(IsPlayerInEvent(playerid))
	{
		...
		Servers(playerid, "You're spawn to the event.");
...
stock eventLeave(playerid)
{
	...
	if(pData[playerid][pAdminDuty]) SetPlayerSkin(playerid, pData[playerid][pSkin]), SetPlayerColor(playerid, COLOR_RED);
	else if (pData[playerid][pOnDuty]) SetFactionColor(playerid), SetPlayerSkin(playerid, pData[playerid][pFacSkin]);
	else SetPlayerSkin(playerid, pData[playerid][pSkin]), SetPlayerColor(playerid, COLOR_WHITE);
...
hook OnPlayerText(playerid, text[])
{
	...
	{
		foreach(new i : Player) if(IsPlayerInEvent(i) && eventTeams[playerid] == eventTeams[i]) {
			SendClientMessageEx(i, eventTeams[playerid] == TEAM_A ? COLOR_RED : COLOR_BLUE, "%s: "WHITE_E"%s", ReturnName(playerid), text);
		}
	}
	...
	if (killerid != INVALID_PLAYER_ID) {
		if (IsPlayerInEvent(playerid)) {
			...
			if(eventData[eventScore][eventTeams[killerid]-1] == eventData[eventTarget]) {
				...
				SendEventMessageAll(COLOR_ARWIN, sprintf("EVENT: "WHITE_E"Event telah selesai dan "YELLOW_E"team %s "WHITE_E"sebagai pemenang.", (eventTeams[killerid] == TEAM_A) ? ("A") : ("B")));

				foreach(new i : Player) if(IsPlayerInEvent(i))
				{
					if(eventTeams[i] == eventData[eventWinner]) GivePlayerMoneyEx(i, eventData[eventBonus]*3), SendCustomMessage(i, "EVENT", "Team anda menang dalam event ini dan mendapat hadiah sebanyak {00FF00}%s.", FormatMoney(eventData[eventBonus]*3));
					else GivePlayerMoneyEx(i, eventData[eventBonus]), SendCustomMessage(i, "EVENT", "Kamu mendapat bonus {00FF00}%s "WHITE_E"dari partisipasi event.", FormatMoney(eventData[eventBonus]));
...
hook OnPlayerSpawn(playerid)
{
	if (IsPlayerInEvent(playerid)) {
		...
		} else SetPlayerColor(playerid, COLOR_BLUE);
		eventSpawn(playerid);
	}
...
CMD:event(playerid, params[])
{
	...
	{
		if(pData[playerid][pAdmin] > 4) return Usage(playerid, "/event [create/end/open/start/weapon/spawn1/spawn2/message]");
		else return Usage(playerid, "/event [join/leave/respawn]");
	}
	if(!strcmp(option, "message")) {
		...
		if(sscanf(extendstring, "s[128]", message))
			return Usage(playerid, "/event message [text]");
		...
		SendClientMessageToAllEx(COLOR_ARWIN, "EVENT: "RED_E"%s "WHITE_E"mengadakan %s event, "YELLOW_E"/event join "WHITE_E"untuk berpartisipasi!.", pData[playerid][pAdminname], EventType(eventData[eventType]));
	}
		...
		if(!eventData[eventOpen])
			return Error(playerid, "Buka partisipasi event terlebih dahulu dengan perintah "WHITE_E"'/event open'.");
		...
		SendClientMessageToAllEx(COLOR_ARWIN, "EVENT: "RED_E"%s "WHITE_E"memulai event "RED_E"%s"WHITE_E", tidak dapat bergabung bagi yang belum masuk ke dalam event.", pData[playerid][pAdminname], EventType(eventData[eventType]));
	}
	else if(!strcmp(option, "join")) {
		if(pData[playerid][pInjured])
			return Error(playerid, "You're on injured mode.");
		...
		if(eventData[eventType] == TYPE_TDM)
		{
			...
			SendCustomMessage(playerid, "EVENT", "Kamu memasuki event sebagai team %s"WHITE_E", Kamu akan melihat player lain saat event dimulai.", eventTeams[playerid] == TEAM_A ? (""RED_E"A") : ("{0049FF}B"));
		}
		else if(eventData[eventType] == TYPE_JETPACK)
		{
			...
			{
				case 1: SetPlayerPos(playerid, 726.0457,-1460.6995,22.2109), SetPlayerFacingAngle(playerid, 178.90); 
				case 2: SetPlayerPos(playerid, -1513.5300,-397.7400,7.0781), SetPlayerFacingAngle(playerid, 135.09);
				case 3: SetPlayerPos(playerid, 1748.9951,2318.4055,22.8222), SetPlayerFacingAngle(playerid, 90.66);
            }
		...
		if (!IsPlayerInEvent(playerid))
			return Error(playerid, "Kamu tidak berada di dalam event.");

		if(eventWaitingSpawn[playerid]) {
			eventSpawn(playerid);
		} else {
			Error(playerid, "Kamu sudah spawn ke dalam event.");
		}
	}
		...
		SendEventMessageAll(COLOR_ARWIN, sprintf("EVENT: "WHITE_E"%s menghentikan event yang sedang berlangsung.", pData[playerid][pAdminname]));
...
Dialog:eventBonus(playerid, response, listitem, inputtext[])
{
	if(response) {
		...
		if(eventData[eventType] == TYPE_TDM)
		{
			Dialog_Show(playerid, eventFinal, DIALOG_STYLE_LIST, "Event Information", "Score Target\t"YELLOW_E"%d"WHITE_E"\nSkin Team 1\t"YELLOW_E"%d"WHITE_E"\nSkin Team 2\t"YELLOW_E"%d"WHITE_E"\nHealth Spawn\t"YELLOW_E"%.1f"WHITE_E"\nArmor Spawn\t"YELLOW_E"%.1f\n"WHITE_E"Event Bonus\t{00FF00}%s", "Start", "Back", 
	...
	{
		if(eventData[eventType] == TYPE_TDM) Dialog_Show(playerid, eventHealth, DIALOG_STYLE_INPUT, "Event "RED_E"Health", "Masukkan value darah yang akan di berikan:", "Set", "Back");
...
Dialog:jetpackLoc(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		...
		Info(playerid, "Kamu di tempatkan di spawn jetpack pertama.");
		...
		Dialog_Show(playerid, eventFinal, DIALOG_STYLE_LIST, "Event Information", "Event Type\t"YELLOW_E"Jetpack"WHITE_E"\nEvent Location\t"YELLOW_E"%s"WHITE_E"\nEvent Bonus\t{00FF00}%s", "Start", "Back", 
			loc,
			FormatMoney(eventData[eventBonus])
		);
	}
	else Dialog_Show(playerid, eventBonus, DIALOG_STYLE_INPUT, "Event {00FF00}Bonus", ""GREEN_E"Pada event jetpack, bonus hanya untuk partisipasi, bonus pemenang di beri admin secara manual\n\n"WHITE_E"Masukkan berapa bonus yang akan di berikan:", "Set", "Back");
	return 1;
}
...
Dialog:eventFinal(playerid, response, listitem, inputtext[])
{
	if(response) {
		...
		Servers(playerid, "You've create the event, /event open to open the event.");
	}
...
hook OnPlayerEnterRaceCP(playerid)
{
	if(IsPlayerInEvent(playerid) && eventData[eventType] == TYPE_JETPACK)
	{
		...
		if(eventCheckpoint[playerid] == (eventData[eventJetpackType]*16)) 
		{
			...
			SendEventMessageAll(COLOR_ARWIN, sprintf("EVENT: "WHITE_E"Event "RED_E"Jetpack "WHITE_E"telah selesai dan dimenangkan oleh "YELLOW_E"%s", ReturnName(playerid)));
			SendAdminMessage(COLOR_RED, "AdmWarn: Event Jetpack dimenangkan oleh %s", ReturnName(playerid));

			foreach(new i : Player) if(IsPlayerInEvent(i))
			{
				if(eventData[eventWinner] == playerid) SendCustomMessage(playerid, "EVENT", "Kamu menang dalam event "RED_E"jetpack."), GivePlayerMoneyEx(playerid, eventData[eventBonus]), SendCustomMessage(playerid, "EVENT", "Kamu menang dan mendapat hadiah {00FF00}%s "WHITE_E"dari jetpack event.", FormatMoney(eventData[eventBonus]));
				else GivePlayerMoneyEx(playerid, 100), SendCustomMessage(playerid, "EVENT", "Kamu mendapat bonus {00FF00}%s "WHITE_E"dari partisipasi jetpack event.", FormatMoney(eventData[eventBonus]));
