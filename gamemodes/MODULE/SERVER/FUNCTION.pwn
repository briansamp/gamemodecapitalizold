...
function OnPlayerDataLoaded(playerid, race_check)
{
	...
	if(cache_num_rows() > 0)
	{
		...
		new string[512];
		format(string, sizeof string, "{FFFFFF}Welcome To Capitaliz Roleplay\n{FFFFFF}Account: {00FF00}%s\n{FFFFFF}Status: {71FF00}Verifed\n{ffffff}Mainkan GTA Roleplay secukupnya jangan lupa istirahat\n{FFFFFF}Enter your password:", pData[playerid][pName]);
...
function CheckBan(playerid)
{
	if(cache_num_rows() > 0)
	{
		...
		else
		{
			...
			{
				new lstr[512];
				format(lstr, sizeof(lstr), "{FF0000}You are banned from this server!\n\n"LB2_E"Ban Info:\n{FF0000}Name: {778899}%s\n{FF0000}IP: {778899}%s\n{FF0000}Admin: {778899}%s\n{FF0000}Ban Date: {778899}%s\n{FF0000}Ban Reason: {778899}%s\n{FF0000}Ban Time: {778899}Permanent\n\n{FFFFFF}Feel that you were wrongfully banned? Appeal at https://dsc.gg/Capitaliz Roleplay", BannedName, PlayerIP, PlayerName, ReturnDate(banDate), Reason);
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""RED_E"BANNED", lstr, "Exit", "");
			}
			else
			{
				new lstr[512];
				format(lstr, sizeof(lstr), "{FF0000}You are banned from this server!\n\n"LB2_E"Ban Info:\n{FF0000}Name: {778899}%s\n{FF0000}IP: {778899}%s\n{FF0000}Admin: {778899}%s\n{FF0000}Ban Date: {778899}%s\n{FF0000}Ban Reason: {778899}%s\n{FF0000}Ban Time: {778899}%s\n\n{FFFFFF}Feel that you were wrongfully banned? Appeal at https://dsc.gg/Capitaliz Roleplay", BannedName, PlayerIP, PlayerName, ReturnDate(banDate), Reason, ReturnTimelapse(gettime(), banTime_Int));
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""RED_E"BANNED", lstr, "Exit", "");
			}
			KickEx(playerid);
			return 1;
...
function AssignPlayerData(playerid)
{
	...
	cache_get_value_name_int(0, "inflat", pData[playerid][pInFlat]);
	cache_get_value_name_int(0, "inhotel", pData[playerid][pInHotel]);
	...
	cache_get_value_name_int(0, "paracetamol", pData[playerid][pParacetamol]);
	cache_get_value_name_int(0, "amoxicilin", pData[playerid][pAmoxicilin]);
	cache_get_value_name_int(0, "antasida", pData[playerid][pAntasida]);
	...
	cache_get_value_name_int(0, "fishtool", pData[playerid][pPancingan]);
	cache_get_value_name_int(0, "fish", pData[playerid][pBeratIkan]);
	cache_get_value_name_int(0, "worm", pData[playerid][pCacing]);
	//
	cache_get_value_name_int(0, "fmax", pData[playerid][pFMax]);
	cache_get_value_name_int(0, "ftime", pData[playerid][pFTime]);
	//
	...
	//trucker
	cache_get_value_name_int(0, "truckerelic", pData[playerid][pTruckerLic]);
	cache_get_value_name_int(0, "truckerlic_time", pData[playerid][pTruckerLicTime]);
	//
	...
	cache_get_value_name_int(0, "togreports", pData[playerid][pTogReports]);
	cache_get_value_name_int(0, "togdamage", pData[playerid][pTogDamage]);
	...
	cache_get_value_name_float(0, "bodycondition0", pData[playerid][pBodyCondition][0]);
	cache_get_value_name_float(0, "bodycondition1", pData[playerid][pBodyCondition][1]);
	cache_get_value_name_float(0, "bodycondition2", pData[playerid][pBodyCondition][2]);
	cache_get_value_name_float(0, "bodycondition3", pData[playerid][pBodyCondition][3]);
	cache_get_value_name_float(0, "bodycondition4", pData[playerid][pBodyCondition][4]);
	cache_get_value_name_float(0, "bodycondition5", pData[playerid][pBodyCondition][5]);
...
function LoadStats(playerid, PlayersName[])
{
	...
	else
	{
		...
		format(gstr,sizeof(gstr),"%sKelamin: [%s] | Uang: ["GREEN_E"%s"WHITE_E"] | Bank: ["GREEN_E"%s"WHITE_E"] | Rekening Bank: [%d] | Nomor Telepon: [None]\n", gstr,(gender == 2) ? ("Perempuan") : ("Laki-laki") , FormatMoney(money), FormatMoney(bmoney), brek);
		format(gstr,sizeof(gstr),"%sTanggal lahir: [%s] | Job: [None] | Job2: [None] | Faction: [%s] | Family: [%s]\n\n", gstr, age, fac, fname);
		...
		format(gstr,sizeof(gstr),"%sAdmin: [%s"WHITE_E"] | Time Played: [%d hour(s) %d minute(s) %02d second(s)] | Gold Coin: ["YELLOW_E"%d"WHITE_E"]\n", gstr, name, jam, menit, detik, coin);
		if(vip != 0)
		{
...
function DragUpdate(playerid, targetid)
{
    if(pData[targetid][pDragged] && pData[targetid][pDraggedBy] == playerid)
    {
		...
		ApplyAnimation(targetid,"PED", "FLOOR_HIT",4.1,1,1,1,1,1);
    }
    return 1;
}
...
function ppCountDown(playerid)
{
	...
	if(0 >= Count)
	{
		Count = -1;
		KillTimer(countTimer);
		if(showCD[playerid] == 1)
  		{
   			GameTextForPlayer(playerid, "~w~GO~r~!~g~!~b~!", 2500, 6);
	...
	else
	{
		if(showCD[playerid] == 1)
 		{
			GameTextForPlayer(playerid, CountText[Count-1], 2500, 6);
...
public settime(playerid)
{
	...
	format(datestring, sizeof datestring, "%s%d:%s%d:%s%d", (hours < 10) ? ("0") : (""), hours-1, (minutes < 10) ? ("0") : (""), minutes, (seconds < 10) ? ("0") : (""), seconds);
...
{
  	new String[10000];
  	format(String,sizeof(String),"(Audio Plugin) %s(%d) has connected to the audio server.", pData[playerid][pName], playerid);
...
{
  	new String[10000];
  	format(String,sizeof(String),"(Audio Plugin) %s(%d) has disconnected from the audio server.", pData[playerid][pName],playerid);
  	SendClientMessageEx(playerid, 0xA9C4E4FF, String);
  	return 1;
}
...
{
	new stocks = RandomEx(4100, 7521);
	new stockss = RandomEx(4120, 5670);
	new stocksss = RandomEx(4150, 6579);
	Material = stocks;
	Component = stockss;
	GasOil = stocksss;
	Product = stocks;
	ServerCargoSeed = stockss;
	ServerCargoMeat = stocksss;
	Food = stocks;
	Server_Save();
	return 1;
}
...
{	
	...
	if(up_seconds == 60)
	{
	    ...
	    if(up_minutes == 60)
	    {
			...
			WorldWeather = rand;
			// Sync Server
			mysql_tquery(g_SQL, "OPTIMIZE TABLE `bisnis`,`houses`,`flats`,`hotels`,`toys`,`vehicle`");
	...
	foreach(new ii : Player)
	{
		...
		if(pData[ii][pTruckerLic] > 0)
		{
			...
			{
				Info(ii, "Masa berlaku {FFFF00}Trucker License {FFFFFF}anda telah habis, silahkan perpanjang kembali!");
