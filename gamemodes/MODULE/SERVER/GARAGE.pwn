...
CMD:buygarage(playerid, params[])
{
	...
	if ((id = Garage_Nearest(playerid)) != -1)
	{
		...
		SendClientMessageEx(playerid, COLOR_GREEN, "PROPERTY: {FFFFFF}You've successfully buy a Garage for {009000}%s", FormatMoney(GarageData[id][garagePrice]));
	}
	return 1;
}
...
CMD:destroygarage(playerid, params[])
{
	...
	if (sscanf(params, "d", id))
	    return Error(playerid, "/destroygarage [garage id]");

	if ((id < 0 || id >= MAX_GARAGE) || !GarageData[id][garageExists])
	    return Error(playerid, "You have specified an invalid Garage ID.");

	Garage_Delete(id);
	Servers(playerid, "You have successfully destroyed Garage ID: %d.", id);
	return 1;
}
...
CMD:editgarage(playerid, params[])
{
	...
	if (sscanf(params, "ds[24]S()[128]", id, type, string))
 	{
	 	SendAdminMessage(playerid, "/editgarage [id] [name]");
	    SendAdminMessage(playerid, "/editgarage LOCATION PRICE");
		return 1;
	}
	if ((id < 0 || id >= MAX_GARAGE) || !GarageData[id][garageExists])
	    return Error(playerid, "You have specified an invalid Garage ID.");
	...
	else if (!strcmp(type, "price", true))
	{
	    ...
	    if (sscanf(string, "d", price))
	        return Usage(playerid, "/editgarage [id] [price] [new price]");
...
CMD:asellgarage(playerid, params[])
{
	...
	if (sscanf(params, "d", id))
	    return Usage(playerid, "/asellgarage [Garage ID]");
	...
	Servers(playerid, "You have sold Garage ID: %d.", id);
	return 1;
}
