...
AddDamageToPlayer(playerid, weaponid, bodypart, issuerid) {
    ...
    if((id = GetSameDamageInfo(playerid, weaponid, bodypart)) != INVALID_DAMAGE_ID) {
        ...
        if((id = GetDamageFreeID(playerid)) != INVALID_DAMAGE_ID) {
		    ...
		    format(DamageData[playerid][id][DAMAGE_ISSUER], MAX_PLAYER_NAME, pData[issuerid][pName]);
...
ResetPlayerDamage(playerid) {
    for(new i = 0; i < MAX_LISTED_DAMAGE; i++) {
		...
		format(DamageData[playerid][i][DAMAGE_ISSUER], MAX_PLAYER_NAME, "");
    }
}
...
ShowDamageToPlayer(playerid, show_to_id) {
    ...
    if(pData[show_to_id][pAdmin] < 1)
    {
        ...
        return ShowPlayerDialog(show_to_id, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, "Damagelog", string, "Ok", "");
    }
    else 
    {
        new str[MAX_PLAYER_NAME];
        format(string, sizeof(string), "Bullet(s)\tBodypart\tShooter(s)\tLast Updated\n");
        for(new i = 0; i < MAX_LISTED_DAMAGE; i++) if(DamageData[playerid][i][DAMAGE_EXISTS]) {

            GetWeaponName(DamageData[playerid][i][DAMAGE_WEAPONID], weapon_name, 24);

            format(str, sizeof(str), "Damagelog %s", pData[playerid][pName]);
            format(string, sizeof(string), "%s%d bullet\t%s\t%s\t%s\n", 
                string, DamageData[playerid][i][DAMAGE_AMOUNT], GetBodyPartName(DamageData[playerid][i][DAMAGE_BODYPART]), DamageData[playerid][i][DAMAGE_ISSUER], ReturnDuration(gettime() - DamageData[playerid][i][DAMAGE_TIME]));
        }
        return ShowPlayerDialog(show_to_id, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, str, string, "Ok", "");
    }
}
...
CMD:checkdamage(playerid, params[])
{
	...
	{
	    Usage(playerid, "/checkdamage <ID/Name>");
	    return true;
	}
    ...
    ShowDamageToPlayer(otherid, playerid);
    return 1;
}