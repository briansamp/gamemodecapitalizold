...
//reload
function Reload(playerid, gunid, ammo)
{
	if(!IsPlayerConnected(playerid)) return 0;
	if(!IsValidTimer(pData[playerid][pReload])) return 0;
	//if(gunid == 0 || ammo == 0) return 0;
	if(pData[playerid][pActivityTime] >= 100)
	{
		new queens[266];
		format(queens, sizeof(queens), "Megazine/%s.txt", pData[playerid][pName]);
		if(!dini_Exists(queens))
  		{
  			Error(playerid, "You Not Dont Have Megazine!");
    		KillTimer(pData[playerid][pReload]);
  		...
  		else
    	{
    		new weaponid = GetPlayerWeaponEx(playerid);
  			new queen[266];
			format(queen, sizeof(queen), "Megazine/%s.txt", pData[playerid][pName]);
  			dini_Remove(queen);
      		GivePlayerWeaponEx(playerid, weaponid, 100);
			Info(playerid, "Your Success Reloaded");
			KillTimer(pData[playerid][pReload]);
			pData[playerid][pActivityTime] = 0;
			HidePlayerProgressBar(playerid, pData[playerid][activitybar]);
			PlayerTextDrawHide(playerid, ActiveTD[playerid]);
...
CMD:weapon(playerid, params[])
{
	...
	if(sscanf(params, "s[20]S()[128]", name, give))
		return Usage(playerid, "/weapon [give/drop/pickup/pos/bone/hide/reload]");
	...
	else if(!strcmp(name, "reload", true))
	{
		if (!weaponid)
			return Error(playerid, "You are not holding a weapon.");
		...
		{
			Info(playerid, "Anda Reload Menggunakan 1 Megazine!");
			SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, "** %s Mereload senjata menggunakan magazine.", ReturnName(playerid));
			ApplyAnimation(playerid, "BD_FIRE", "wash_up", 4.1, 0, 0, 0, 0, 0, 1);
			pData[playerid][pReload] = SetTimerEx("Reload", 1000, true, "idd", playerid);
			PlayerTextDrawSetString(playerid, ActiveTD[playerid], "Reload...");
