...
Atm_Refresh(id)
{
    if(id > -1)
    {
		...
		AtmData[id][atmLabel] = CreateDynamic3DTextLabel(str, COLOR_JOB, AtmData[id][atmX], AtmData[id][atmY], AtmData[id][atmZ] + 0.3, 10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 0, AtmData[id][atmWorld], AtmData[id][atmInt], -1, 10.0);
    }
}
...
CMD:editatm(playerid, params[])
{
    ...
    if(sscanf(params, "ds[24]S()[128]", id, type, string))
    {
        ...
        SCM(playerid, COLOR_JOB, "OPSI: "WHITE_E"[posisi] [status]");
        return 1;
    }
...
CMD:atm(playerid, params[])
{
	...
	{
		new tstr[250];
		format(tstr, sizeof(tstr), ""ORANGE_E"No Rek: "LB_E"%d\n", pData[playerid][pBankRek]);
		format(tstr, sizeof(tstr), "%s{3498db}» {FFFFFF}Balance: $%s\n", tstr, FormatMoney(pData[playerid][pBankMoney]));
		format(tstr, sizeof(tstr), "%s{3498db}» {FFFFFF}Withdraw Money -\n", tstr);
		format(tstr, sizeof(tstr), "%s{3498db}» {FFFFFF}Transfer Money -\n", tstr);
		format(tstr, sizeof(tstr), "%s{3498db}» {FFFFFF}Sign Paycheck -\n", tstr);
		ShowPlayerDialog(playerid, DIALOG_ATM, DIALOG_STYLE_TABLIST_HEADERS, "{FFFFFF}ATM : Menu", tstr, "Oke", "Exit");
	}
	return 1;
}
