...
stock Business_PointCargo(playerid)
{
	foreach(new ii : Bisnis)	
	{
		if(IsPlayerInRangeOfPoint(playerid, 3.0, bData[ii][bsPos][0], bData[ii][bsPos][1], bData[ii][bsPos][2]))
		{
			return ii;	
		}
	}
...
Bisnis_Refresh(id)
{
    if(id != -1)
    {
		...
		if(IsValidDynamic3DTextLabel(bData[id][bsLabel]))
            DestroyDynamic3DTextLabel(bData[id][bsLabel]);

        if(IsValidDynamicPickup(bData[id][bsPickup]))
            DestroyDynamicPickup(bData[id][bsPickup]);
		...
		bData[id][bsPickup] = CreateDynamicPickup(1239, 23, bData[id][bsPos][0], bData[id][bsPos][1], bData[id][bsPos][2]+0.2, 0, 0, _, 8.0);
		
		format(tstr, 128, "[ID: %d]\n"WHITE_E"Cargo Point\nType {FFFF00}'/cargo sell' {ffffff}to sell your cargo", id);
		bData[id][bsLabel] = CreateDynamic3DTextLabel(tstr, COLOR_LOGS, bData[id][bsPos][0], bData[id][bsPos][1], bData[id][bsPos][2]+0.5, 30.0);
 	...
 	if(rows)
  	{
		for(new i; i < rows; i++)
		{
			...
			cache_get_value_name_float(i, "pcx", bData[bid][bsPos][0]);
			cache_get_value_name_float(i, "pcy", bData[bid][bsPos][1]);
			cache_get_value_name_float(i, "pcz", bData[bid][bsPos][2]);
			cache_get_value_name_int(i, "cargo", bData[bid][bCargo]);
...
CMD:createbisnis(playerid, params[])
{
	...
	bData[bid][bsPos][0] = 0.0;
	bData[bid][bsPos][1] = 0.0;
	bData[bid][bsPos][2] = 0.0;
...
CMD:gotobisnis(playerid, params[])
{
	...
	SendClientMessageEx(playerid, COLOR_PINK, "You has teleport to bisnis id %d", bid);
...
CMD:editbisnis(playerid, params[])
{
    ...
    if(sscanf(params, "ds[24]S()[128]", bid, type, string))
    {
        ...
        SendClientMessage(playerid, COLOR_YELLOW, "[NAMES]:{FFFFFF} location, interior, locked owner, pointcargo, point, price, type, product, restock, reset");
        return 1;
    }
    ...
    if(!strcmp(type, "location", true))
    {
        ...
        SendAdminMessage(COLOR_PINK, "%s has adjusted the location of bisnis ID: %d.", pData[playerid][pAdminname], bid);
    }
    else if(!strcmp(type, "interior", true))
    {
        ...
        SendAdminMessage(COLOR_PINK, "%s has adjusted the interior spawn of bisnis ID: %d.", pData[playerid][pAdminname], bid);
    }
    else if(!strcmp(type, "locked", true))
    {
        ...
        if(locked) {
            SendAdminMessage(COLOR_PINK, "%s has locked bisnis ID: %d.", pData[playerid][pAdminname], bid);
        }
        else {
            SendAdminMessage(COLOR_PINK, "%s has unlocked bisnis ID: %d.", pData[playerid][pAdminname], bid);
        }
    }
    else if(!strcmp(type, "price", true))
    {
        ...
        Bisnis_Save(bid);
		Bisnis_Refresh(bid);
        SendAdminMessage(COLOR_PINK, "%s has adjusted the price of bisnis ID: %d to %d.", pData[playerid][pAdminname], bid, price);
    }
	else if(!strcmp(type, "type", true))
    {
        ...
        Bisnis_Save(bid);
		Bisnis_Refresh(bid);
        SendAdminMessage(COLOR_PINK, "%s has adjusted the type of bisnis ID: %d to %d.", pData[playerid][pAdminname], bid, btype);
    }
	else if(!strcmp(type, "product", true))
    {
        ...
        Bisnis_Save(bid);
		Bisnis_Refresh(bid);
        SendAdminMessage(COLOR_PINK, "%s has adjusted the product of bisnis ID: %d to %d.", pData[playerid][pAdminname], bid, prod);
    }
	else if(!strcmp(type, "money", true))
    {
        ...
        Bisnis_Save(bid);
		Bisnis_Refresh(bid);
        SendAdminMessage(COLOR_PINK, "%s has adjusted the money of bisnis ID: %d to %s.", pData[playerid][pAdminname], bid, FormatMoney(money));
    }
	else if(!strcmp(type, "restock", true))
    {
		...
		if(prod == 0)
		{
			...
			Bisnis_Save(bid);
			Bisnis_Refresh(bid);
			SendAdminMessage(COLOR_PINK, "%s has adjusted the restock of bisnis ID: %d to disable.", pData[playerid][pAdminname], bid);
		}
		else if(prod == 1)
		{
			...
			Bisnis_Save(bid);
			Bisnis_Refresh(bid);
			SendAdminMessage(COLOR_PINK, "%s has adjusted the restock of bisnis ID: %d to enable.", pData[playerid][pAdminname], bid);
		}
    ...
    else if(!strcmp(type, "owner", true))
    {
        ...
        Bisnis_Save(bid);
		Bisnis_Refresh(bid);
        SendAdminMessage(COLOR_PINK, "%s has adjusted the owner of bisnis ID: %d to %s", pData[playerid][pAdminname], bid, owners);
    }
    //tot
    else if(!strcmp(type, "song", true))
    {
        ...
        if(sscanf(string, "s[128]", song))
            return Usage(playerid, "/editbisnis [id] [song]");

        format(bData[bid][bSong], 128, song);
  
        Bisnis_Save(bid);
		Bisnis_Refresh(bid);
        SendAdminMessage(COLOR_PINK, "%s has adjusted the song of bisnis ID: %d to %s", pData[playerid][pAdminname], bid, song);
    }
    else if(!strcmp(type, "reset", true))
    {
		...
		Bisnis_Save(bid);
		Bisnis_Refresh(bid);
        SendAdminMessage(COLOR_PINK, "%s has reset bisnis ID: %d.", pData[playerid][pAdminname], bid);
    }
	else if(!strcmp(type, "point", true))
    {
		...
		Bisnis_Save(bid);
		Bisnis_Refresh(bid);
        SendAdminMessage(COLOR_PINK, "%s has edit bisnis point ID: %d.", pData[playerid][pAdminname], bid);
    }
	else if(!strcmp(type, "pointcargo", true))
    {
		new Float:x, Float:y, Float:z;
        GetPlayerPos(playerid, x, y, z);
		bData[bid][bsPos][0] = x;
		...
		Bisnis_Save(bid);
		Bisnis_Refresh(bid);
        SendAdminMessage(COLOR_PINK, "%s has edit bisnis point cargo ID: %d.", pData[playerid][pAdminname], bid);
    }
	else if(!strcmp(type, "delete", true))
    {
        ...
        DestroyDynamicPickup(bData[bid][bsPickup]);
        DestroyDynamic3DTextLabel(bData[bid][bsLabel]);
		...
		bData[bid][bPickup] = -1;
		bData[bid][bsLabel] = Text3D: INVALID_3DTEXT_ID;
		bData[bid][bsPickup] = -1;
		bData[bid][bsPos][0] = 0.0;
		bData[bid][bsPos][1] = 0.0;
		bData[bid][bsPos][2] = 0.0;
		bData[bid][bCargo] = 0;
        ...
        SendAdminMessage(COLOR_PINK, "%s has delete bisnis ID: %d.", pData[playerid][pAdminname], bid);
	}
    return 1;
}
...
CMD:mybis(playerid)
{
	...
	ShowPlayerDialog(playerid, DIALOG_MY_BISNIS, DIALOG_STYLE_LIST, "{FF0000}HB:PR {0000FF}Bisnis", CMDSString, "Select", "Cancel");
	return 1;
}
