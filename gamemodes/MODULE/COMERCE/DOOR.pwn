...
Doors_Updatelabel(id)
{
	if(id != -1)
	{
		...
		if(dData[id][dGarage] == 1)
		{
			...
			dData[id][dPickupext] = CreateDynamicPickup(954, 23, dData[id][dExtposX], dData[id][dExtposY], dData[id][dExtposZ], dData[id][dExtvw], dData[id][dExtint]);
        ...
        if(dData[id][dIntposX] != 0.0 && dData[id][dIntposY] != 0.0 && dData[id][dIntposZ] != 0.0)
        {
			if(dData[id][dGarage] == 1)
			{
				...
				dData[id][dPickupint] = CreateDynamicPickup(954, 23, dData[id][dIntposX], dData[id][dIntposY], dData[id][dIntposZ], dData[id][dIntvw], dData[id][dIntint], -1, 7);
...
CMD:createdoor(playerid, params[])
{
	...
	dData[did][dPickupext] = CreateDynamicPickup(954, 23, dData[did][dExtposX], dData[did][dExtposY], dData[did][dExtposZ], dData[did][dExtvw], dData[did][dExtint]);
...
CMD:editdoor(playerid, params[])
{
    ...
    if(!strcmp(type, "location", true))
    {
        ...
        SendAdminMessage(COLOR_PINK, "%s has adjusted the location of entrance ID: %d.", pData[playerid][pAdminname], did);
    }
    else if(!strcmp(type, "interior", true))
    {
       ...
       /*foreach (new i : Player)
        ...
        SendAdminMessage(COLOR_PINK, "%s has adjusted the interior spawn of entrance ID: %d.", pData[playerid][pAdminname], did);
    }
    else if(!strcmp(type, "custom", true))
    {
        ...
        if(status) {
            SendAdminMessage(COLOR_PINK, "%s has enabled custom interior mode for entrance ID: %d.", pData[playerid][pAdminname], did);
        }
        else {
            SendAdminMessage(COLOR_PINK, "%s has disabled custom interior mode for entrance ID: %d.", pData[playerid][pAdminname], did);
        }
    }
    else if(!strcmp(type, "virtual", true))
    {
        ...
        Doors_Save(did);
		Doors_Updatelabel(did);
        SendAdminMessage(COLOR_PINK, "%s has adjusted the virtual of entrance ID: %d to %d.", pData[playerid][pAdminname], did, worldid);
    }
    else if(!strcmp(type, "password", true))
    {
        ...
        Doors_Save(did);
		Doors_Updatelabel(did);
        SendAdminMessage(COLOR_PINK, "%s has adjusted the password of entrance ID: %d to %s", pData[playerid][pAdminname], did, password);
    }
    else if(!strcmp(type, "locked", true))
    {
        ...
        if(locked) {
            SendAdminMessage(COLOR_PINK, "%s has locked entrance ID: %d.", pData[playerid][pAdminname], did);
        } else {
            SendAdminMessage(COLOR_PINK, "%s has unlocked entrance ID: %d.", pData[playerid][pAdminname], did);
        }
    }
    else if(!strcmp(type, "name", true))
    {
        ...
        SendAdminMessage(COLOR_PINK, "%s has adjusted the name of entrance ID: %d to \"%s\".", pData[playerid][pAdminname], did, ColouredText(name));
    }
	else if(!strcmp(type, "admin", true))
    {
        ...
        SendAdminMessage(COLOR_PINK, "%s has set entrance ID: %d to admin level %d.", pData[playerid][pAdminname], did, level);
    }
	else if(!strcmp(type, "vip", true))
    {
        ...
        SendAdminMessage(COLOR_PINK, "%s has set entrance ID: %d to VIP level %d.", pData[playerid][pAdminname], did, level);
    }
	else if(!strcmp(type, "faction", true))
    {
        ...
        SendAdminMessage(COLOR_PINK, "%s has set entrance ID: %d to faction id %d.", pData[playerid][pAdminname], did, fid);
    }
	else if(!strcmp(type, "family", true))
    {
        ...
        SendAdminMessage(COLOR_PINK, "%s has set entrance ID: %d to family id %d.", pData[playerid][pAdminname], did, fid);
    }
	else if(!strcmp(type, "garage", true))
	{
		...
		{
			dData[did][dGarage] = 0;
			SendAdminMessage(COLOR_PINK, "%s has set entrance ID: %d to garage vehicle disable.", pData[playerid][pAdminname], did);
		}
		else
		{
			dData[did][dGarage] = 1;
			SendAdminMessage(COLOR_PINK, "%s has set entrance ID: %d to garage vehicle enable.", pData[playerid][pAdminname], did);
		}
		Doors_Save(gid);
	...
	else if(!strcmp(type, "delete", true))
    {
        ...
        SendAdminMessage(COLOR_PINK, "%s has delete door ID: %d.", pData[playerid][pAdminname], did);
	}
    return 1;
}
